...
from examples.ex_3_theatres_beds.simulation_execution_functions import (
...
st.set_page_config(
    layout="wide", initial_sidebar_state="expanded", page_title="Orthopaedic Ward - HEP"
)
...
with col_b:
    st.dataframe(
        ...
        .rename(columns={0: "Sessions"})
        .merge(
            pd.DataFrame.from_dict(
                schedule.theatres_per_weekday, orient="index"
...
with col2:
    ...
    los_delay_sd = st.slider(
        "Variation of delay (standard deviation)", 1.0, 25.0, 15.2, 0.1
    )
...
show_operation_type = anim_param_col_2.selectbox(
    ...
    [
        "Show knee vs hip",
        "Show revision vs primary",
        "Show both",
        "Show standard patient icons",
    ],
    index=0,
)
...
if button_run_pressed:
    ...
    def generate_animation_fig():
        ...
        primary_patients["patient class"] = primary_patients[
            "patient class"
        ].str.title()
        primary_patients["ID"] = (
            primary_patients["ID"].astype("str") + primary_patients["patient class"]
        )

        revision_patients = results[3]
        revision_patients = revision_patients[revision_patients["rep"] == 1]
        revision_patients["patient class"] = revision_patients[
            "patient class"
        ].str.title()
        revision_patients["ID"] = (
            revision_patients["ID"].astype("str") + revision_patients["patient class"]
        )

        full_log_with_patient_details = (
            event_log.merge(
                pd.concat([primary_patients, revision_patients]),
                how="left",
        ...
        pid_table = (
            full_log_with_patient_details[["patient"]]
            .drop_duplicates()
            .reset_index(drop=True)
        ...
        full_log_with_patient_details = (
            full_log_with_patient_details.merge(pid_table, how="left", on="patient")
            .drop(columns="patient")
            .rename(columns={"pid": "patient"})
        )
        ...
        event_position_df = pd.DataFrame(
            [
                ...
                # Triage - minor and trauma
                {
                    ...
                    "x": 200,
                    "y": 650,
                    "label": "Waiting for<br>Availability of<br>Bed to be Confirmed<br>Before Surgery",
                },
                {
                    "event": "no_bed_available",
                    "x": 600,
                    "y": 650,
                    "label": "No Bed<br>Available:<br>Surgery Cancelled",
                },
                {
                    ...
                    "y": 220,
                    "resource": "n_beds",
                    "label": "In Bed:<br>Recovering from<br>Surgery",
                },
                {
                    "event": "discharged_after_stay",
                    "x": 670,
                    "y": 50,
                    "label": "Discharged from Hospital<br>After Recovery",
                },
                # {'event': 'exit',
        ...
        if show_operation_type == "Show both":
            full_patient_df_plus_pos = full_patient_df_plus_pos.assign(
                icon=full_patient_df_plus_pos.apply(set_icon_full, axis=1)
            )
        elif show_operation_type == "Show knee vs hip":
            full_patient_df_plus_pos = full_patient_df_plus_pos.assign(
                icon=full_patient_df_plus_pos.apply(set_icon_surgery_target, axis=1)
            )
        elif show_operation_type == "Show revision vs primary":
            full_patient_df_plus_pos = full_patient_df_plus_pos.assign(
                icon=full_patient_df_plus_pos.apply(set_icon_surgery_type, axis=1)
            )
        else:
            full_patient_df_plus_pos = full_patient_df_plus_pos.assign(
                icon=full_patient_df_plus_pos.apply(set_icon_standard, axis=1)
            )
        ...
        if show_delayed_discharges:
            full_patient_df_plus_pos = full_patient_df_plus_pos.assign(
                icon=full_patient_df_plus_pos.apply(indicate_delay_via_icon, axis=1)
            )
        ...
        cancelled_due_to_no_bed_available = len(
            full_log_with_patient_details[
                full_log_with_patient_details["event"] == "no_bed_available"
            ]["patient"].unique()
        )
        ...
        st.markdown(
            f"Surgeries cancelled due to no bed being available in time: {cancelled_perc:.2%} ({cancelled_due_to_no_bed_available} of {total_patients})"
        )
        ...
        counts_not_avail = (
            full_patient_df_plus_pos[
                full_patient_df_plus_pos["event"] == "no_bed_available"
            ]
            .sort_values("snapshot_time")[["snapshot_time", "patient"]]
        ...
        counts_not_avail = (
            ...
            .merge(
                full_patient_df_plus_pos[["snapshot_time"]].drop_duplicates(),
                how="right",
            )
        ...
        counts_not_avail["running_total"] = counts_not_avail.sort_values(
            "snapshot_time"
        )["patient"].cumsum()
        ...
        counts_ops_completed = (
            full_patient_df_plus_pos[
                full_patient_df_plus_pos["event"] == "post_surgery_stay_begins"
        ...
        # Calculate a running total of this value, which will be used to add the correct value
        # to each individual frame
        counts_ops_completed["running_total"] = counts_ops_completed.sort_values(
            "snapshot_time"
        )["patient"].cumsum()
        counts_not_avail = counts_not_avail.merge(
            counts_ops_completed.rename(columns={"running_total": "completed"}),
            how="left",
            on="snapshot_time",
        )
        counts_not_avail["perc_slots_lost"] = counts_not_avail["running_total"] / (
        ...
        sp = make_subplots(
            ...
            subplot_titles=("", "Daily lost slots"),
        )
        ...
        fig.update_layout(yaxis2=dict(title="Operations Cancelled<br>Per Day"))
    ...
    def display_animation_fig(fig):
        ...
        frame_duration = frame_duration_col.number_input(
            "Choose the frame duration in milliseconds (default is 1500ms)",
            50,
            5000,
            1500,
        )
        frame_transition_duration = frame_transition_duration_col.number_input(
            "Choose the length of the transition between frames in milliseconds (default is 1000ms)",
            0,
            5000,
            1000,
        )
