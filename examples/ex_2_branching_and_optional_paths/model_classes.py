...
class Scenario:
    ...
    def init_sampling(self):
        ...
        self.exam_dist = Normal(
            self.exam_mean, np.sqrt(self.exam_var), random_seed=self.seeds[2]
        )
...
class NonTraumaPathway(object):
    ...
    def execute(self):
        ...
        if self.require_treat:
            ...
            self.wait_treat = self.env.now - start_wait
            trace(f"treatment of patient {self.identifier} begins {self.env.now:.3f}")
            self.event_log.append(
                {
                    "patient": self.identifier,
                    "pathway": "Non-Trauma",
...
class TreatmentCentreModel:
    ...
    def init_resources(self):
        ...
        for i in range(self.args.n_triage):
            self.args.triage.put(
                CustomResource(self.env, capacity=1, id_attribute=i + 1)
            )
        ...
        for i in range(self.args.n_exam):
            self.args.exam.put(CustomResource(self.env, capacity=1, id_attribute=i + 1))
    ...
    def run(self, results_collection_period=60 * 24 * 10):
        ...
        self.env.process(
            self.interval_audit_utilisation(resources=resources_list, interval=5)
        )
                    ...
                    self.utilisation_audit.append(
                        {
                            ...
                            # The number of users
                            "number_utilised": len(
                                resources[i]["resource_object"].items
                            ),
                            "number_available": resources[i][
                                "resource_object"
                            ].capacity,
